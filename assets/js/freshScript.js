...
function createWorks() {
  workArray.push({
    ...
    job_description: document.getElementById("job_description_input_id").value,
  });
    ...
    <div class="form_2_added_details_card">
      <div class="added_details_left">
        ...
        <div class="added_details_desc">
          <p><span>Job Title:</span> <span id='titleId${index}'>${
      item.job_title
    }</span></p>
          <p><span>Employer:</span> <span id='employerId${index}'>${
      item.employer
    }</span> </p>
          <p ><span>City:</span> <span id='cityId${index}'>${
      item.city
    } </span></p>
          <p ><span>Country:</span><span id='countryId${index}'> ${
      item.country
    }</span></p>
          <p> <span>Start Date/End Date: </span> <span id='startDateId${index}'> ${
      item.start_date
    }</span><span id='endDateId${index}'>${item.end_date}</span> </p>
...
function editWork(clicked_id) {
  ...
  document
    .getElementById("work_xperience_status_id")
    .classList.add("hide_work_btn");
  ...
  for (let i = 0; i < workArray.length; i++) {
    ...
    if (i == splitBtnInd[0]) {
      document.getElementById("job_title_id").value = document.getElementById(
        `titleId${splitBtnInd[0]}`
      ).innerHTML;
      document.getElementById("employer_id").value = document.getElementById(
        `employerId${splitBtnInd[0]}`
      ).innerHTML;
      document.getElementById("city_id_form_2").value = document.getElementById(
        `cityId${splitBtnInd[0]}`
      ).innerHTML;
      document.getElementById("country_id_form_2").value =
        document.getElementById(`countryId${splitBtnInd[0]}`).innerHTML;
      document.getElementById("start_date_id").value = document.getElementById(
        `startDateId${splitBtnInd[0]}`
      ).innerHTML;
      document.getElementById("end_date_id").value = document.getElementById(
        `endDateId${splitBtnInd[0]}`
      ).innerHTML;
...
function createSchoolData() {
  schoolArray.push({
    ...
    field_of_study: document.getElementById("field_study_id").value,
    ...
    const markup = `
      <div class="form_2_added_details_card">
        <div class="added_details_left">
          ...
          <div class="added_details_desc">
            <p><span>School name:</span> <span id='schoolNameId${index}'>${
      item.school_name
    }</span> </p>
            <p><span>Field of study:</span> <span id='fieldNameId${index}'>${
      item.field_study
    }</span> </p>
            <p><span>Degree:</span> <span id='degreeId${index}'>${
      item.degree
    }</span> </p>
            <p><span>City:</span> <span id='cityNameId${index}' >${
      item.city
    }</span></p>
            <p><span>Country:</span> <span id='countryNameId${index}'> ${
      item.country
    }</span></p>
            <p>
...
function editEducation(eduClickedId) {
  ...
  for (let i = 0; i < schoolArray.length; i++) {
    if (i == eduEditBtnInd[1]) {
      document.getElementById("school_name_id").value = document.getElementById(
        `schoolNameId${eduEditBtnInd[1]}`
      ).innerHTML;
      document.getElementById("degree_type_id").value = document.getElementById(
        `fieldNameId${eduEditBtnInd[1]}`
      ).innerHTML;
      document.getElementById("city_id_form_4").value = document.getElementById(
        `cityNameId${eduEditBtnInd[1]}`
      ).innerHTML;
      document.getElementById("country_id_form_4").value =
        document.getElementById(`countryNameId${eduEditBtnInd[1]}`).innerHTML;
      document.getElementById("start_date_form_4_id").value =
        document.getElementById(`schoolStartDate${eduEditBtnInd[1]}`).innerHTML;
      document.getElementById("end_date_form_4_id").value =
        document.getElementById(`schoolEndDate${eduEditBtnInd[1]}`).innerHTML;
    }
  }
  if (eduEditBtnInd[1] > -1) {
    // only splice array when item is found
